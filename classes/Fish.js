function Fish(ctx, x, y, radius, color, tailLength, tailWidth) {
	...
	this.x = x;
	this.y = y;
	this.velY = 100 + Math.random() * 250; // pixels per second
	this.velX = 100 + Math.random() * 250; // pixels per second
...
Fish.prototype.tick = function(modifier) {
...
Fish.prototype.move = function(modifier) {
	// the modifier (1/fps) helps us keep our speed constant despite framerate
	this.x -= Math.round(modifier * (this.velX * Math.random() + this.velX / 2));
	this.y += Math.round(modifier * (Math.cos(this.ticks * .5) * this.velY * Math.random() ));
}
