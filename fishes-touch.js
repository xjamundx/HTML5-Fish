...
function reportCrash(fish, i) {
	var transform = "translateY(-"+Math.floor(fish.y)+1+"px)";
	score++;
	$("#score").text("Death Toll: " + score);
	fish.el.style.webkitTransition = "-webkit-transform 10s ease-out";
	fish.el.style.webkitTransform = transform;
	fish.el.style.mozTransform = transform;
	delete(fishes[i]);
}
...
fish.prototype.move = function() {
	...
	this.dy = Math.cos(Date.now()* Math.PI) * 7;
...
$(function() {
	...
	$("#stage").bind("touchmove", {}, function(e) {
		...
		if (window.event.targetTouches && window.event.targetTouches.length < 2) return false
	});
